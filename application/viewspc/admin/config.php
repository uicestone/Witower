...
<div id="right" class="span9">
	...
	<div class="model">
		...
		<div class="main">
			...
			<div class="title"> is_audit值：0代表所有评论都不需要经过审核即可显示  ，  1代表必须经过后台审核才可以显示评论  </div>
<!--			<div class="title"><h3><a href="/<?= uri_segment(1) ?>/config">评论是否需要审核</a></h3></div>
			<form method="post" enctype="multipart/form-data">
				<table class="table table-bordered">
					<tbody>
						<tr>
							<td>
							<input name="is_audit" <? if($config_items['is_audit'] == 0){ ?>checked<? } ?> type="radio" value="0" name="url">不必审核
							<input name="is_audit" <? if($config_items['is_audit'] == 1){ ?>checked<? } ?> type="radio" value="1" name="url">必须要审核
							</td>
							<td><button type="submit" class="btn btn-small">提交</button></td>
						</tr>
					</tbody>
				</table>
			</form>
