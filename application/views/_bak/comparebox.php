...
<body>
<div class="o-v" id="compare">
	<div class="edition">
		<ul class="l">
			<!--{if $edition[1]['time']}-->
			<li><b>{lang editionPre}</b> <label><?=$edition[1]['time']?></label></li>
			<li>
			{lang commentUser}<a href="{url user-space-$edition[1]['authorid']}"><?=$edition[1]['author']?></a>
			{lang worldnum}<label><?=$edition[1]['words']?>{lang piece}</label>
			{lang image}<label><?=$edition[1]['images']?>{lang piece}</label>
			</li>
			<!--{else}-->
			<li><b>{lang editionNoCompare}</b></li>
			<!--{/if}-->
		</ul>
		<ul class="r">
			<li><b>{lang editionWaitoAccred}</b>	<label><?=$edition[0]['time']?></label></li>
			<li>
			{lang commentUser}<a href="{url user-space-$edition[0]['authorid']}"><?=$edition[0]['author']?></a>
			{lang worldnum}<label><?=$edition[0]['words']?>{lang piece}</label>
			{lang image}<label><?=$edition[0]['images']?>{lang piece}</label>
			</li>
		</ul>
	</div>
	
	<div id="LeftScroll" class="LeftFrame" onscroll="DriveRightScroll()">
		<span id="LeftContent" align="left"><?=$edition[1]['content']?></span>
	</div>
	<div id="RightScroll" class="RightFrame" onscroll="DriveLeftScroll()">
		<span id="RightContent" align="left"><?=$edition[0]['content']?></span>
	</div>
	
	<script type="text/javascript">CompareById('LeftContent', 'RightContent');</script>
	<p class="main l mar-t8">
